...
async function main(): Promise<void> {
  const [revision, signalsPath, labelsPath, reportPath] = process.argv.slice(2);
  ...
  const results: Record<string,unknown> = {revision,frames:frames.length,faceFrames:frames.filter(frame=>frame.faceDetected).length};
  for(const [name, Machine, config] of [
    ...
    for(const frame of frames){const result=machine.process(frame);if(result.event)events.push(result.event);}
